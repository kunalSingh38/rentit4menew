    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        for (int i = 0; i < jsonDecode(response.body)['Response'].length; i++) {
          countrycodelistwithname.add(
              "${jsonDecode(response.body)['Response'][i]['name']} +${jsonDecode(response.body)['Response'][i]['dialcode']}");
          countrycodelist
              .add("+${jsonDecode(response.body)['Response'][i]['dialcode']}");
          countrynamelist
              .add(jsonDecode(response.body)['Response'][i]['name'].toString());
        }
      });
    }
  }
